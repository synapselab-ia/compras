...
-- F10 design proof only. Everything here is disposable and artificial.
-- No object created by this file is a production migration.
...
-- A security-invoker projection is safe but insufficient: the existing RLS
-- still exposes only the caller's own membership/app_user.
...
SELECT test_support_directory.assert_count(
  'SELECT count(*) FROM public.team_member_directory_invoker_probe',
  1,
...
-- The intuitive same-table policy is rejected by PostgreSQL as recursive.
...
-- Preferred design probe: a sealed NOLOGIN/NOBYPASS capability owns only the
-- security-barrier view. It receives column-minimized base grants and targeted
-- read policies, never ownership of protected base tables.
...
-- ALTER VIEW requires the target owner to be able to own objects in the schema.
-- The CREATE privilege is temporary and removed immediately after ownership transfer.
...
SELECT test_support_directory.expect_sqlstate(
  'SET ROLE compras_directory_view_owner_probe',
  '42501',
  'operational role cannot set capability role'
);

-- Active A1 sees only active/non-disabled directory entries in team A.
BEGIN;
SET LOCAL request.jwt.claims = '{"iss":"https://directory.demo.invalid","sub":"DEMO-DIR-A1"}';
SELECT test_support_directory.assert_count(
  'SELECT count(*) FROM public.team_member_directory_capability_probe',
  2,
...
-- The targeted capability policies do not widen direct access for the caller.
SELECT test_support_directory.assert_count(
  'SELECT count(*) FROM public.memberships',
  1,
  'caller direct memberships remains self-only after capability policies'
);
SELECT test_support_directory.assert_count(
  'SELECT count(*) FROM public.app_users',
  1,
  'caller direct app_users remains self-only after capability policies'
);
ROLLBACK;
...
-- Known app_user without membership receives no directory.
BEGIN;
...
-- Disabled caller cannot resolve current_app_user_id(), even with membership row.
BEGIN;
...
-- Unknown external identity also fails closed.
BEGIN;
