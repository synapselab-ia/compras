...
**PROJECT_STATUS:** F30_PR_READY_F31_PLANNED  
**CURRENT_PHASE:** F30 implementada e verde na PR `#46`, aguardando promoção; F31 preparada como única próxima ação após integração; F21 ON HOLD  
**REPO_VISIBILITY:** PUBLIC  
**APPLICATION_STATUS:** HOSTED_DEMO_AVAILABLE_SELF_HOSTED_AUTH_SIGNIN_LIMITER_NEXT_ACTION_WRITE_CREATE_BOUNDARY_AND_F30_CREATE_UI_PR_READY  
...
**F30_PR:** `#46` — OPEN / READY AFTER GREEN GATES  
**F30_FUNCTIONAL_HEAD:** `38f7683bc5c3f7ab539a5764d3f1088dfd499f60`  
**F30_PR_CI_RUN:** `34700039223` — PASS  
**F30_PR_F22_PREFLIGHT_RUN:** `34700039200` — PASS  
**F30_PR_F29_CREATE_RUN:** `34700039212` — PASS  
...
A sessão recuperou `main` em `baee340772c00283b19114411b7ecd13c770396b`, confirmou ausência de PR/branch F30 ativa, revalidou os 10 blobs do `CONTEXT_MANIFEST` e confirmou F29 integrada/verde. A única frente canônica era F30.
...
## F30 — jornada mínima de criação persistente

F30 conecta a UI à boundary F29 sem criar uma nova autoridade de persistência.
...
- o formulário inicial recebe candidate UUID opaco preparado server-side por `preparePersistentContractingCandidateId()`;
...
O red-team identificou que regenerar candidate após um resultado técnico incerto enfraqueceria a proteção de retry da ADR-012. A implementação foi corrigida antes da promoção:

- `created` e `already-created` seguem para o detalhe do UUID validado;
- `not-available` e `unavailable` retornam ao formulário com somente o estado sanitizado + o mesmo candidate UUID validado;
- a rota reutiliza esse candidate somente se ele for UUID válido; valor malformado é descartado e um novo candidate server-side é preparado;
- candidate continua não sendo authority e não revela team/actor;
- o botão usa `useFormStatus` para bloquear repetição acidental enquanto a action está pendente.

Feedback de criação é aceito apenas pelos estados `created`, `already-created`, `not-available` e `unavailable`; texto arbitrário de query não é refletido na UI.
...
Gates no head funcional `38f7683bc5c3f7ab539a5764d3f1088dfd499f60`:

- CI `34700039223`: PASS, incluindo lint, typecheck, unit/component tests, build, database e auth-database;
- F22 Private Preview Preflight `34700039200`: PASS;
- F29 Contracting Create `34700039212`: PASS.

A PR ainda deve repetir os gates após este checkpoint documental antes do merge. Após promoção, `main` deve ser validada novamente e este arquivo deve registrar merge/post-merge.
...
F31 é exclusivamente de desenho: deve produzir a decisão arquitetural da próxima mutação de `contractings.object`, sem implementar migration, SQL, Server Action ou UI nessa work unit.
