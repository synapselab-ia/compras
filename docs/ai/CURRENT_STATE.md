...
O primeiro run dedicado encontrou uma ambiguidade PL/pgSQL entre variável local e coluna `actor_membership_id`. A variável foi renomeada para `current_actor_membership_id`, sem relaxamento de segurança. O red-team foi então ampliado e o head promovido ficou verde.

Na verificação de checkpoint pós-merge, o workflow F41 detectou quoting inválido em três asserções recém-adicionadas de `database/tests/related_identifier_create.sql`: `$...$` havia sido materializado como `$...# Current State - Compras
...
. O defeito estava restrito ao teste adversarial. Migration `0010`, primitive, grants, policies, provisioning e adapter não foram alterados. O quoting foi corrigido e a suíte completa ficou verde no head `c61eb4a3c253e96fb347b2787d201bc5e249c36f`.
...
Após a correção exclusivamente de teste no checkpoint, o head `c61eb4a3c253e96fb347b2787d201bc5e249c36f` ficou verde em CI `35356582294`, F22 `35356582129`, F29 `35356581998`, F32 `35356582261`, F35 `35356582179`, F38 `35356582177` e F41 `35356581973`.

O conector GitHub disponível expõe runs associados a evento de pull request, mas não fornece readback dos runs de push do merge commit. Nenhum PASS de push pós-merge é afirmado sem evidência. `LAST_GOOD_MAIN_COMMIT` continua representando o último merge de `main` cujo run de push pôde ser lido diretamente pelo conector.
