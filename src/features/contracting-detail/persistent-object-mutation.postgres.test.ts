...
const integrationEnabled = process.env.F32_CONTRACTING_OBJECT_POSTGRES_TEST === "1";
const describePostgres = integrationEnabled ? describe : describe.skip;

const TARGET_ID = "32040000-0000-4000-8000-000000000090";
const TEAM_ID = "32010000-0000-4000-8000-000000000090";
const USER_ID = "32020000-0000-4000-8000-000000000090";
const MEMBERSHIP_ID = "32030000-0000-4000-8000-000000000090";
const ISSUER = "urn:compras:better-auth:self-hosted:v1";
const SUBJECT = "DEMO-F32-CONCURRENT";
const INITIAL_OBJECT = "  DEMO F32 concurrent object old  ";

describePostgres("F32 PostgreSQL contracting object concurrency boundary", () => {
  let runtimePool: Pool;
  let adminPool: Pool;

  beforeAll(async () => {
    const runtimeUrl = process.env.F32_DOMAIN_RUNTIME_DATABASE_URL;
    const adminUrl = process.env.F32_ADMIN_DATABASE_URL;

    if (!runtimeUrl || !adminUrl) {
      throw new Error("F32 PostgreSQL integration references are required");
    }
    ...
    await adminPool.query(
      `insert into public.teams (id, name, created_at)
       values ($1::uuid, 'DEMO-F32-Team-Concurrent', '2026-01-01T00:00:00Z')`,
      [TEAM_ID],
    );
    await adminPool.query(
      `insert into public.app_users (
         ...
         $1::uuid, $2, $3, 'DEMO F32 Concurrent', '2026-01-01T00:00:00Z', null
       )`,
    ...
    await adminPool.query(
      `insert into public.contractings (
         ...
         $1::uuid, $2::uuid, $3, 'DEMO F32 next invariant',
         '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z'
       )`,
      [TARGET_ID, TEAM_ID, INITIAL_OBJECT],
    );
  });
  ...
  beforeEach(async () => {
    ...
    await adminPool.query(
      `update public.contractings
          set object = $2,
              next_action = 'DEMO F32 next invariant',
              updated_at = '2026-01-01T00:00:00Z'
        where id = $1::uuid`,
      [TARGET_ID, INITIAL_OBJECT],
    );
  });
  ...
  async function mutate(candidate: number): Promise<string> {
    ...
    const eventId = `32050000-0000-4000-8000-${candidate.toString().padStart(12, "0")}`;
    const newObject = `  DEMO F32 concurrent winner ${candidate}  `;
    ...
    try {
      ...
      const result = await client.query<{ outcome: string }>(
        ...
        [TARGET_ID, INITIAL_OBJECT, newObject, eventId],
      );
  ...
  it("serializes writers so exactly one update/event wins and the rest conflict", async () => {
    ...
    const persisted = await adminPool.query<{
      ...
      `select
         ...
         count(event.id) filter (
           where event.old_value = $2
             and event.new_value = contracting.object
             and event.actor_membership_id = $3::uuid
             and event.team_id = $4::uuid
             and event.event_type = 'object_changed'
             and event.field_key = 'object'
      ...
      [TARGET_ID, INITIAL_OBJECT, MEMBERSHIP_ID, TEAM_ID],
    );

    expect(persisted.rows[0]?.object).toMatch(/^  DEMO F32 concurrent winner [1-8]  $/);
    expect(persisted.rows[0]?.next_action).toBe("DEMO F32 next invariant");
